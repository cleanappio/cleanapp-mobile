...
class PendingShareStore(private val context: Context) {
  ...
  fun saveDraft(report: SharedIncomingReport) {
    val persisted = report.copy(localImagePath = persistImageIfNeeded(report))
  ...
  fun removeDraft(report: SharedIncomingReport) {
    ...
    persistDrafts(drafts)
    val imagePath = report.localImagePath?.trim()?.takeIf { it.isNotEmpty() } ?: return
  ...
  private fun persistImageIfNeeded(report: SharedIncomingReport): String? {
    val imagePath = report.localImagePath?.trim()?.takeIf { it.isNotEmpty() } ?: return null
    ...
    val destination = File(imagesDir(), "${report.id}.$extension")
  ...
  private fun JSONObject.toSharedIncomingReport(): SharedIncomingReport =
    SharedIncomingReport(
      ...
      localImagePath = optString("localImagePath").takeIf { it.isNotBlank() },
